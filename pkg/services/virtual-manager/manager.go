...
func (vm *VirtualManager) Run() {
	if len(vm.cfg.VirtualManagerPath) == 0 { //判断VirtualManagerPath是否为空
	...
	if err := os.MkdirAll(vm.cfg.VirtualManagerPath, DEFAULT_DIR_MODE); err != nil && !os.IsNotExist(err) {
		//创建VirtualManagerPath目录
		klog.Fatalf("can't create %s, error %s", vm.cfg.VirtualManagerPath, err)
	}

	registered := make(chan struct{})
	//执行vDeviceWatcher，遍历使用vgpu的running pod，
	//在每个pod目录对应一个vDevice的grpc server，并放入每隔1min循环检测pod目录，
	//如果不存在即停止grpc server
	go vm.vDeviceWatcher(registered)
	<-registered

	go vm.garbageCollector()
	//执行垃圾收集器，进行pod垃圾目录回收;遍历目录下的pod目录，
	//从所有使用vgpu的running pod中查找，
	//不存在即删除对应的pod目录/etc/gpu-manager/vm/
	go vm.process()
	//执行process，遍历VCudaRequestsQueue channel，
	//获取对应事件记录的podUID，
	//再次执行这个过程： 在每个pod目录对应一个vDevice的grpc server，
	//并放入/etc/gpu-manager/vm/DeviceServers map数据结构
		...
		for _, cont := range pod.Spec.Containers {
			if cont.Name == name || strings.HasPrefix(name, utils.MakeContainerNamePrefix(cont.Name)) {
				...
				if policy.IsPolicyPod(pod) {
					cores = policy.GetRealGPU(pod, cont.Name, cores)
				}
...
func (vm *VirtualManager) UpdateGPU(oldObj, newObj interface{}) {
	...
	// 非调度策略调度的 Pod
	if !policy.IsPolicyPod(pod) || !policy.IsPolicyPod(oldPod) {
		return
	}

	// 非请求 GPU 资源的 Pod
	if !utils.IsGPUAssignedPod(pod) || !utils.IsGPUAssignedPod(oldPod) {
		return
	}

	klog.V(2).Infof("try update vcuda cores for %s", pod.Name)

	for i := range pod.Spec.Containers {
		container := &pod.Spec.Containers[i]
		if !utils.IsGPURequiredContainer(container) {
			continue
		}

		oldCores := policy.GetRealGPU(oldPod, container.Name, 0)
		newCores := policy.GetRealGPU(pod, container.Name, 0)
		// 算力没有发生变化
		if oldCores == newCores {
			continue
		}

		contID := ""
		for _, stat := range pod.Status.ContainerStatuses {
			if strings.HasPrefix(stat.Name, container.Name) {
				contID = stat.ContainerID
				break
			}
		}

		contID = strings.TrimPrefix(contID, "docker://")
		if len(contID) == 0 {
			klog.Errorf("can't locate %s(%s)", string(pod.UID), container.Name)
			return
		}

		klog.V(2).Infof("try update vcuda cores %d to %d for %s/%s", oldCores, newCores, pod.UID, container.Name)

		// 调整容器的算力
		if err := vm.updateVDeviceWithContainer(string(pod.UID), contID, container.Name); err != nil {
			klog.Errorf("can't update vcuda cores for %s/%s: %s", pod.UID, container.Name, err.Error())
			return
		}

		klog.V(2).Infof("Successfull update vcuda cores %d to %d for %s/%s", oldCores, newCores, pod.UID, container.Name)
	}
}

func (vm *VirtualManager) updateVDeviceWithContainer(podUID, contID, contName string) error {
	klog.V(2).Infof("UID: %s, cont: %s want to registration", podUID, contID)

	resp := vm.responseManager.GetResp(podUID, contName)
	if resp == nil {
		return fmt.Errorf("unable to load allocResp for %s/%s", podUID, contID)
	}

	baseDir := utils.GetVirtualControllerMountPath(resp)
	if baseDir == "" {
		return fmt.Errorf("unable to find virtual manager controller path")
	}

	configFilename := filepath.Join(baseDir, contID, CONTROLLER_CONFIG_NAME)
	if _, err := os.Stat(configFilename); err != nil {
		// 当 Pod 中的容器运行起来后, 但仍未开始使用 GPU, 则此时该容器对应的目录是不存在的,
		// 针对这种情况, 这里不会返回错误.
		if os.IsNotExist(err) {
			klog.Warningf("vcuda config file %s not exists for %s/%s", configFilename, podUID, contID)
			return nil
		}
		return fmt.Errorf("unable to stat config file %s for %s/%s, err: %s", podUID, contID, configFilename, err)
	}

	if err := vm.updateConfigFile(configFilename, podUID, contName); err != nil {
		return err
	}
...
func (vm *VirtualManager) updateConfigFile(filename string, podUID, name string) error {
	activePods := watchdog.GetActivePods()
	pod, ok := activePods[podUID]
	if !ok {
		return fmt.Errorf("can't locate %s", podUID)
	...
	for _, cont := range pod.Spec.Containers {
		if cont.Name == name || strings.HasPrefix(name, utils.MakeContainerNamePrefix(cont.Name)) {
			...
			if policy.IsPolicyPod(pod) {
				cores = policy.GetRealGPU(pod, cont.Name, cores)
				klog.Infof(fmt.Sprintf("getRealGpu cores %d", cores))
			}
