...
import (
	...
	vmapi "tkestack.io/gpu-manager/pkg/api/runtime/virtual-manager"
	...
	"tkestack.io/gpu-manager/pkg/policy"
...
func kubeInit() {
	...
	if nodeName == "" {
		klog.Fatalln("Please set env NODE_NAME")
	}
}
...
func (ta *NvidiaTopoAllocator) capacity() (devs []*pluginapi.Device) {
	...
	//khw add
	klog.V(2).Infof("TotalMemoryBlocks Is %v , TotalMemory %v ", totalMemoryBlocks, totalMemory)
...
func (ta *NvidiaTopoAllocator) patchGPUMemry(resourceName string) error {

	var totalMemory int64
	var memoryDevices []*pluginapi.Device
	nodes := ta.tree.Leaves()
	for i := range nodes {
		totalMemory += int64(nodes[i].Meta.TotalMemory)
	}
	...
	node, err := ta.k8sClient.CoreV1().Nodes().Get(nodeName, metav1.GetOptions{})
	if err != nil {
		return err
	}

	if val, ok := node.Status.Capacity[v1.ResourceName(resourceName)]; ok {
		if val.Value() == int64(totalMemoryBlocks) {
			klog.Infof("No need to update Capacity %s", v1.ResourceName(resourceName))
			return nil
		}
	}

	newNode := node.DeepCopy()
	newNode.Status.Capacity[v1.ResourceName(resourceName)] = *resource.NewQuantity(int64(totalMemoryBlocks), resource.DecimalSI)
	newNode.Status.Allocatable[v1.ResourceName(resourceName)] = *resource.NewQuantity(int64(totalMemoryBlocks), resource.DecimalSI)
	...
	_, err = ta.k8sClient.CoreV1().Nodes().UpdateStatus(newNode)
	if err != nil {
		klog.Infof("Failed to update Capacity %s.", resourceName)
	} else {
		klog.Infof("Updated Capacity %s successfully.", resourceName)
	}
...
func (ta *NvidiaTopoAllocator) allocateOne(pod *v1.Pod, container *v1.Container, req *pluginapi.ContainerAllocateRequest) (*pluginapi.ContainerAllocateResponse, error) {
	...
	if allocated {
		...
		default:
			...
			////htl 修改点
			//var gpuIdx int
			//if utils.IsGpuSpecifiedModelPod(pod) {
			//	klog.V(2).Infof("container %s of pod %s specified gpu model.", container.Name, pod.UID)
			//	containerIndex, err := utils.GetContainerIndexByName(pod, container.Name)
			//	if err != nil {
			//		return nil, err
			//	}
			//	idx := utils.GetContainerGpuIdx(pod, containerIndex)
			//	gpuIdx, _ = strconv.Atoi(idx)
			//}

			//修改点
			//if policy.IsPolicyPod(pod) {
			//	klog.Info(fmt.Sprintf("preEvaluate needCores %d ", needCores))
			//	needCores = policy.GetRealGPU(pod, container.Name, needCores)
			//}
	...
	for _, n := range nodes {
		...
		fmt.Printf("Allocate %s for %s(%s), Meta (%d:%d)", name, pod.UID, container.Name, n.Meta.ID, n.Meta.MinorID)
	...
	if !allocated {
		...
		ta.allocatedPod.Insert(string(pod.UID), container.Name, pod, &cache.Info{
	...
	if shareMode {
		...
		// khw add LD_PRELOAD
		ctntResp.Envs["LD_PRELOAD"] = "/usr/local/nvidia/lib64/libdlsym.so:/usr/local/nvidia/lib64/libcuda-control.so"
...
func (ta *NvidiaTopoAllocator) ListAndWatchWithResourceName(resourceName string, e *pluginapi.Empty, s pluginapi.DevicePlugin_ListAndWatchServer) error {
	...
	// khw add
	klog.V(2).Infof("ListAndWatch %s , Devs Number Is %+v", resourceName, iNumber)
...
func (ta *NvidiaTopoAllocator) PreStartContainer(ctx context.Context, req *pluginapi.PreStartContainerRequest) (*pluginapi.PreStartContainerResponse, error) {
	...
	// 第二个修改点
	if policy.IsPolicyPod(pod) {
		vcore = policy.GetRealGPU(pod, containerName, vcore)
	}

	//修改点,创建容器之前检查剩余算力.
	if policy, ok := pod.Annotations["ideal.com/cluster-policy-name"]; ok {
		klog.Info(fmt.Sprintf("pod policy %v", policy))
		if policy == WeightSchedulerName {
			idx := GetContainerIdxByName(pod, containerName)
			klog.Info(fmt.Sprintf("check pod ann %v container lens %d", pod.GetAnnotations(), len(pod.Spec.Containers)))
			klog.Info(fmt.Sprintf("check gpu card real restCores,pod %s ,containter %s contId %d.", pod.Name, containerName, idx))
			if idx > -1 {
				restCores, err := GetGpuRestCores(ta.k8sClient, pod, idx)
				if err != nil {
					return nil, err
				}

				realCore := int64(0)
				if coreStr, ok := pod.Annotations[types.RealVCoreAnnotationPrefix+strconv.Itoa(idx)]; ok {
					realCore = cast.ToInt64(coreStr)
				}

				klog.Info(fmt.Sprintf("vcore %d compare restCores %d", realCore, restCores))
				if int(realCore) > restCores {
					return nil, fmt.Errorf("pod %s container %s expire cores %d rather than rest recores %d",
						pod.Name, containerName, realCore, restCores)
				}
			}
		}
	}
...
func (ta *NvidiaTopoAllocator) preStartContainerCheck(podUID string, containerName string, vcore int64, vmemory int64) error {
	...
	if c, ok := cache[containerName]; !ok {
		...
		// htl modify 因为存在修改算力的情况暂时把判断去掉，待验证会有什么其他影响
...
func GetContainerIdxByName(pod *v1.Pod, contName string) int {
	for idx, cont := range pod.Spec.Containers {
...
func GetGpuRestCores(cli kubernetes.Interface, pod *v1.Pod, contIdx int) (int, error) {
	// 获取指定节点上指定 GPU 卡已运行进程的信息
	klog.Info(fmt.Sprintf("precheck GetGpuRestCores. container idx %d", contIdx))
	gpuid := ""
	if id, ok := pod.Annotations[types.PredicateGPUIndexPrefix+strconv.Itoa(contIdx)]; ok {
		gpuid = id
	}

	podList, err := cli.CoreV1().Pods(v1.NamespaceAll).List(metav1.ListOptions{})
	if err != nil {
		return 0, err
	}
	...
	for _, tmpPod := range podList.Items {
		//由于pod还没启动这部分的算力暂时跳过.
		if (tmpPod.Name == pod.Name && tmpPod.Namespace == pod.Namespace) || tmpPod.Status.Phase != v1.PodRunning {
			continue
		}
		//running pod

		for key, ann := range tmpPod.GetAnnotations() {
			if strings.HasPrefix(key, types.PredicateGPUIndexPrefix) {
				if ann == gpuid {
					contIdx := strings.Split(key, types.PredicateGPUIndexPrefix)[1]
					cores := tmpPod.GetAnnotations()[types.RealVCoreAnnotationPrefix+contIdx]
					realCores, _ := strconv.Atoi(cores)
					klog.Info(fmt.Sprintf("tmpPod %s contIdx %s,cores  ", tmpPod.Name, contIdx, cores))
					sum += realCores
				}
			}
		}
	...
	restCores := 95 - sum
	klog.Info(fmt.Sprintf("gpuCard %s restCores %d", gpuid, restCores))
	return restCores, nil
...
func (ta *NvidiaTopoAllocator) Vgpuinfo(ctx context.Context, req *vmapi.VgpuInfoRequest) (*vmapi.VgpuInfoResponse, error) {
	if req.CardId == "" {
		return nil, fmt.Errorf("CardId is none.")
	}
	carId, _ := strconv.Atoi(req.CardId)
	taskNums := 0
	modelName := ""
	podItems := make([]*vmapi.PodItem, 0)

	activedPods := watchdog.GetActivePods()
	for poduid, _ := range ta.allocatedPod.PodGPUMapping {
		if _, ok := activedPods[poduid]; !ok {
			continue
		}

		flag := false
		pod := ta.allocatedPod.UidPodMapping[poduid]
		actPod, err := ta.k8sClient.CoreV1().Pods(pod.Namespace).Get(pod.Name, metav1.GetOptions{})
		if err != nil {
			return nil, err
		}

		containers := make([]string, 0)
		for contIdx, cont := range actPod.Spec.Containers {
			gpuid := actPod.Annotations[types.PredicateGPUIndexPrefix+strconv.Itoa(contIdx)]
			if gpuid == strconv.Itoa(carId) {

				for _, sts := range actPod.Status.ContainerStatuses {
					if sts.Ready == true {
						taskNums++
						flag = true
		...
		if flag {
			item := vmapi.PodItem{Name: pod.Name, Namespace: pod.Namespace, Containers: containers}
			podItems = append(podItems, &item)
		}
	}

	for _, leaf := range ta.tree.Leaves() {
		if leaf.Meta.MinorID == carId {
			modelName = leaf.Meta.ModelName
			break
		}
	}

	resp := &vmapi.VgpuInfoResponse{
		ModelName: modelName,
		TaskNums:  int32(taskNums),
		PodItems:  podItems,
	}
