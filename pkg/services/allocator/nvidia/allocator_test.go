...
`
	...
	for _, testCase := range testCase {
		...
		if podCache.GetCache(string(pod.UID)) == nil {
			...
			podCache.Insert(string(pod.UID), testCase.PodName, pod,&cache.Info{
...
`
	...
	for i := 0; i < 6; i++ {
		...
		//fmt.Println(fmt.Sprintf("node MinorName %s,node MinorId %d,modelName %s", node.MinorName(), node.Meta.MinorID, node.Meta.ModelName))
	}
	...
	for   _,cont:= range alloc.allocatedPod.PodGPUMapping{
		for _,v:=range cont{
			fmt.Printf("PodGpuMapping Devices value: %v\n",v.Devices)
		}
	}
