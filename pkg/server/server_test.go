...
func TestGPUInfo(t *testing.T) {
	...
	conn, err := grpc.Dial("192.168.1.180:5679", opts...)
	if err != nil {
		t.Fatalf("grpc dial failed: %s", err.Error())
	}
	client := virtualmanager.NewVirtualManagerClient(conn)
	resp, err := client.Vgpuinfo(context.TODO(), &virtualmanager.VgpuInfoRequest{CardId: "0"})
	if err != nil {
		t.Fatalf("Vgpuinfo failed: %s", err.Error())
	}
