...
//
// IsValidGPUPath 检查路径是否是一个有效的  Nvidia GPU 设备路径.
...
//
// GetGPUData 从 annotations 中获取 cores、memory 和 device 名称.
...
func IsGpuSpecifiedModelPod(pod *v1.Pod) bool {

	if _, ok := pod.ObjectMeta.Annotations[types.GPUMODELNAME]; !ok {
		return ok
	}
	return false
...
func GetContainerGpuIdx(pod *v1.Pod, containerIndex int) (gpuIdx string) {
	gpuIdx = "-1"
	if pod.Annotations == nil {
		return gpuIdx
	}

	if idx, ok := pod.ObjectMeta.Annotations[types.PredicateGPUIndexPrefix+strconv.Itoa(containerIndex)]; ok {
		gpuIdx = idx
	}
