...
func (t *NvidiaTree) parseFromLibrary() error {
	...
	for i := 0; i < int(num); i++ {
		...
		n.Meta.UUID = uuid
		n.Meta.ModelName = strings.ReplaceAll(rawName, " ", "-")
...
func (t *NvidiaTree) PrintGpuinfo() string {
	return t.printGpuNode()
}
...
func (t *NvidiaTree) printGpuNode() string {
	...
	gpuinfos := make([]GpuNodeInfo, 0)

	for _, le := range t.root.tree.Leaves() {
		gpu := GpuNodeInfo{
			NodeName:          nodeName,
			Gpuid:             le.Meta.MinorID,
			ModelName:         le.Meta.ModelName,
			Available:         le.Available(),
			Pids:              len(le.Meta.Pids),
			UsedMemory:        le.Meta.UsedMemory,
			AllocatableCores:  le.AllocatableMeta.Cores,
			AllocatableMemory: le.AllocatableMeta.Memory,
		}
		gpuinfos = append(gpuinfos, gpu)

	}

	res, err := json.Marshal(gpuinfos)
	if err != nil {
		return ""
	}
